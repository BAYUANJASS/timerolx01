...
TOKEN = os.getenv("BOT_TOKEN")  # Token diambil dari environment variable
...
async def handle_izin(update, context, user_id, chat_id, message_id, izin_type, duration):
    ...
    if user_id in user_timers:
        await safe_send_message(context, chat_id, "⏳ Kamu masih punya izin aktif. Gunakan /done untuk menyelesaikannya.", message_id)
        return
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_timers:
        ...
        text = "⚠️ Tidak ada izin aktif. Ketik `/izin` atau 'izin ambil makan' untuk mulai."
...
async def siapa_izin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"✅ Orang yang masih izin:\n{chr(10).join(active_users)}" if active_users else "⚠️ Tidak ada yang sedang izin saat ini."
